...
def get_description(model_name):
    try:
        return config["descriptions"][model_name]
    except Exception as e:
...
try:
    try:
        local_models_list = sorted(
            [ollama.list()["models"][i]["model"] for i in range(len(ollama.list()["models"]))])
        local_models_list = [
        ...
        st.error(
            "Failed to load local models.  Please check your Ollama installation.")
